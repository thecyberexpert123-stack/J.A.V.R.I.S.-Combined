...
| **R-II series** (deep research II → owner-directed "continue" sequence, `TASKS.md` at the combined-repo root) — **IN PROGRESS**: C6 `pass^k` reliability in the eval drivers (ADR-0027, harness + tests only, no tag) · C11 task-journal evidence chain (ADR-0028, **1.21.0**) · C3 doorway watchdog + unit hardening (ADR-0029 accepted D1+D2+D3-opt-in, implementing) | `--runs K` on the M2/M4 drivers with τ-bench's `pass^k` curve; per-run state isolation closed a local-rerun breaker artefact · every journal write hash-linked in-transaction, `jarvis doctor` verdict covers the journal (edit/delete/forge/event-wipe all reported; +0.2 ms per write) | Research: `docs/RESEARCH-agent-construction-and-future-tech-2026.md`; order C6 → C11 → C3 → C1 → C4 → C2 → C7 → C8 → C10 → C9 → C5; security-sensitive items pause at their ADR; **nothing merged, `main` untouched** |
