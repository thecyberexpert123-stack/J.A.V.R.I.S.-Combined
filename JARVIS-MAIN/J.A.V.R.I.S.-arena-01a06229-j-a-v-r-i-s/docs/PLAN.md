...
| **R-II series** (deep research II → owner-directed "continue" sequence, `TASKS.md` at the combined-repo root) — **IN PROGRESS**: C6 `pass^k` reliability in the eval drivers (ADR-0027, harness + tests only, no tag) · C11 task-journal evidence chain (ADR-0028, **1.21.0**) · C3 doorway supervision + opt-in brief confinement (ADR-0029, implemented) · C1 owner argument policy (ADR-0030, **1.22.0**) · C4 environment signals for the briefing + opt-in signal listener (ADR-0031 hybrid, **1.23.0**) · C12 self-authored skills (owner-added item; ADR-0032 **accepted 2026-09-08 — D14 — implementation queued after C2**) · C2 resumable task log (ADR-0033 **proposed 2026-09-08, paused for D15**; measured first: crash leaves `running` + no undo artifact) | `--runs K` on the M2/M4 drivers with τ-bench's `pass^k` curve; per-run state isolation closed a local-rerun breaker artefact · every journal write hash-linked in-transaction, `jarvis doctor` verdict covers the journal (edit/delete/forge/event-wipe all reported; +0.2 ms per write) · doorway `Type=notify` + `WatchdogSec=30` via a stdlib `sd_notify` client, `brief install --harden` 9.6 → 2.0 · narrowing-only owner rules over playbook params at the three `check_argv` sites, `policy lint|show|explain`, undo artefacts carry `origins`, M3 0 escapes under permissive and deny-all files · six sensed facts (battery, link, metered, sleep/shutdown imminent, locked, slept) as lines or holds, `--signals off|sysfs|bus` (default `sysfs`), a stdlib read-only D-Bus wire client (no child processes, `NO_AUTO_START`), `jarvis brief listen` / `jarvis-signals.service` recording events and delivering a held briefing on unlock/resume | Research: `docs/RESEARCH-agent-construction-and-future-tech-2026.md`; order C6 → C11 → C3 → C1 → C4 → C2 → **C12 (owner-added, D14)** → C7 → C8 → C10 → C9 → C5; security-sensitive items pause at their ADR; **nothing merged, `main` untouched** |
