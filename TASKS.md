...
### B-C4 · Environment signals as briefing inputs (propose-only) — *OWNER-Q* (M) — `[!]` **ADR-0031 drafted 2026-09-06, paused for the owner**
Origin: research §5 (logind `PrepareForSleep`/`PrepareForShutdown`, session lock, NetworkManager/UPower, inotify, idle), §5.3 (Horvitz threshold), §5.4 signal table. Primary sources fetched for the ADR: logind D-Bus reference, `busctl(1)`, UPower `UPower`/`Device`, NetworkManager manager interface + `nm-dbus-types`, kernel sysfs ABI (`sysfs-class-power`, `sysfs-class-net`, `sysfs-power`), `clock_gettime(2)`, `systemd.timer(5)`.
- [x] Inspected `brief/engine.py` (`compose` 83 — four sources, zero subprocess, enforced by `test_compose_never_spawns_subprocess`; `decide` 145; `BriefLedger` 169; `desktop_notify` 246; `run_once` 265), `brief/install.py` (`Type=oneshot`, `Persistent=true`, `HARDENING_DIRECTIVES` incl. `ProtectKernelTunables=`/`PrivateUsers=`/`RestrictAddressFamilies=AF_UNIX`), ADR-0021 D1–D5, ADR-0018 "doorway, never an actor", ADR-0017 D3, and the HUD's `telemetry/proc_reader.py::read_battery()` (already reads `/sys/class/power_supply` with a `type == Battery` filter, clamp, status map).
- [x] Fetched and cited the exact interface names: login1 Manager `IdleHint`/`PreparingForSleep`/`PreparingForShutdown` (poll-only — "do not send out PropertyChanged"), Session `LockedHint`; UPower `OnBattery`, Device `Percentage`/`State`/`WarningLevel` (verified, **not used in v1**); NetworkManager `Metered` (`NMMetered` 1 YES / 3 GUESS_YES), `State`, `Connectivity`; sysfs `type`/`status`/`capacity`, `operstate`/`type`, `suspend_stats/success`; `busctl call` with `--auto-start=no --timeout=2`.
- [x] ADR-0031 drafted: **D1** six facts — S1 battery discharging ≤ 20 % (sysfs, HUD-identical filter), S2 no link (sysfs), S3 metered (bus), S4 sleep/shutdown imminent (bus, **hold**), S5 session locked (bus, **hold**), S6 slept since last run (sysfs, ledger only); `IdleHint` recorded only; **D2** `--signals off|sysfs|bus`, default `off` = today byte-for-byte; `sysfs` zero-subprocess; `bus` = one fixed-argv `busctl --system --auto-start=no --timeout=2 call … Properties.Get` per property, never bus-activating a service; **no doorway listener, no inotify**; **D3** holds in `decide()` drop the knock, `latest.md` + ledger still written; **D4** additive `signals`/`holds` keys, `null` = not sensed, HUD untouched, one seam rule (mirror `read_battery()`); **D5** flags rendered into `ExecStart=` at install, no config file; **D6** offline tests with fake sysfs trees + terse `busctl` fixtures + golden byte-identity in `off` mode + five mutation checks; **D7** non-goals (no `Inhibit()`, no methods beyond `Properties.Get`).
- [ ] **Owner decides Q-A** signal set (keep S1–S6? prune S2? add `/run/reboot-required`?) and the 20 % threshold (UPower's own low default is 10 % `[snippet]`).
- [ ] **Owner decides Q-B** default mode: `off` (recommended) or `sysfs`.
- [ ] **Owner confirms Q-C** poll-at-briefing only — no doorway listener, no inotify in this ADR.
- [ ] **Owner decides Q-D** hold semantics: no retry until the next calendar fire (recommended) vs `OnUnitInactiveSec=` re-arm.
- [ ] Owner accepts → implement `brief/signals.py` (sysfs + bus sensors, `Signals` value) + `engine.py` (`sense()` before `compose()`, holds in `decide()`/`run_once()`, additive fields) + `install.py`/`app.py` flags + tests; bump **1.23.0**; docs; TASKS tick with Verified / Not verified / Limits.
- Acceptance: with `--signals off` (default) `latest.md` and stdout are byte-identical to today (golden test); with fixtures, items and holds appear exactly as specified; `test_compose_never_spawns_subprocess` unchanged and `sense(mode="sysfs")` proven subprocess-free; ADR-0017 D3 intact (no execution path added); every argv asserted exactly (`--auto-start=no` present).
- Verification plan: fixture tests + full gate + mutation checks (drop `--auto-start=no`; `null` treated as `false`; loopback counted as a link; `GUESS_NO` treated as metered; hold ignored by `run_once`); **live sensing on the owner's machine** — the five commands are in the ADR ("Verified vs. assumed").
- Sandbox limits (measured 2026-09-06): no system bus (`busctl` 252 present, `Failed to connect to bus: No such file or directory`, rc 1, 6 ms), `/sys/class/power_supply/` empty, `suspend_stats/success` = 0, `/sys/class/net` = `lo` + `eth0` — sysfs *parsing* is testable with fake trees; live sensing is not.
